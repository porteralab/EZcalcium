function ez_motion_correct_2channel_1step_fast(fullvidfile1,fullvidfile2,referenceframe,blocksize)
...
imwrite(redchannel(:,:,1),[fullvidfile2(1:end-4) '_ezmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
imwrite(greenchannel(:,:,1),[fullvidfile1(1:end-4) '_ezmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');

for imagedex=2:num_frames 
    if floor(imagedex/10)==(imagedex/10) %show only every 10 frames progress
        display(['Progress: ' num2str((imagedex)/(num_frames)*100) '%']);
    end
    imwrite(redchannel(:,:,imagedex),[fullvidfile2(1:end-4) '_ezmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','append');
    imwrite(greenchannel(:,:,imagedex),[fullvidfile1(1:end-4) '_ezmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','append');
