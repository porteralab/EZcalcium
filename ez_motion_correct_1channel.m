function ez_motion_correct_1channel(fullvidfile,set_motcor)
...
for imagedex = 1:num_frames %For every frame of the video
    ...
    if imagedex==1 %for the first picture, overwrite. Then, append.
        imwrite(secondIdscale(:,:,imagedex),[fullvidfile(1:end-4) '_ezmcor1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
    else %appends the rest of the images to the file in order
        imwrite(secondIdscale(:,:,imagedex),[fullvidfile(1:end-4) '_ezmcor1' '.tif'],'tiff','Compression','none','WriteMode','append');
    end
end
