...
imwrite(firstId(:,:,1),[fullvidfile2(1:end-4) '_ezmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
imwrite(firstIGd(:,:,1),[fullvidfile1(1:end-4) '_ezmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');

for imagedex=2:num_frames 
    if floor(imagedex/10)==(imagedex/10) %show only every 10 frames progress
        display(['Progress: ' num2str((imagedex)/(num_frames)*100) '%']);
    end
    imwrite(firstId(:,:,imagedex),[fullvidfile2(1:end-4) '_ezmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','append');
    imwrite(firstIGd(:,:,imagedex),[fullvidfile1(1:end-4) '_ezmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','append');
